...
for (toPlot in strainList) {
  ...
  toPlot1 <- d2[d2$strainName %in% c(toPlot, controlStrain), ]
...
p1 <- ggplot(toPlot1, aes(x = as.factor(TestNumber), y = DrugMassConsumed, color=DrugDose, group =MouseID)) +          geom_line() + 
      geom_point(shape = 1.5) + 
      theme_classic() + 
...
for (toPlot in strainList) {
  ...
  if(toPlot %in% c("B6NJ-Cyfip2-S968F<J> (GET204)", "C57BL/6J")) next
  # extract the animal to plot
  toPlot1 <- droplevels(as.data.frame(d2[d2$strainName %in% c(toPlot, controlStrain), ]))
...
# Plot sex differences within each strain with significance testing
Compare males vs females at each dose within the same strain, with p-values.

```{r}
strainList <- levels(as.factor(d2$strainName))

outFileNamePDF <- file.path(outputDir, "StrainSexComparison_pValues.pdf")

pdf(outFileNamePDF, 6, 4)
for (toPlot in strainList) {
  tryCatch({

    # extract just this strain (no control comparison)
    toPlot1 <- droplevels(as.data.frame(d2[d2$strainName == toPlot, ]))

    # line plot with sexes as color groups (always printed)
    p1 <- ggline(toPlot1,
                 x = "DrugDose",
                 y = "DrugMassConsumed",
                 add = "mean_se",
    ...
    # run t-tests only for doses where both sexes have >= 2 observations
    tryCatch({
      ...
      if (length(valid_doses) > 0) {
        ...
        if (nrow(stat.test) > 0) {
          # x.position must be the integer index of the factor level for correct alignment
          dose_levels <- levels(factor(toPlot1$DrugDose))
          stat.test$x.position <- match(as.character(stat.test$DrugDose), dose_levels)
          # y.position: max mean+SE across sexes at each dose, with 15% headroom
...
```{r}
################# QC AND OUTLIER IDENTIFICATION (Z-SCORE) ----

ZSCORE_THRESHOLD <- 2.5

# Work on a plain data.frame to avoid data.table/dplyr grouping conflicts
d2_df <- as.data.frame(d2)

# Calculate z-score within each strain + sex + dose group
d2_df$z_score <- ave(
  d2_df$DrugMassConsumed,
  d2_df$strainName, d2_df$sex, d2_df$DrugDose,
  FUN = function(x) {
    s <- sd(x, na.rm = TRUE)
...
# Average z-score per mouse per dose (handles multiple tests at the same dose)
z_per_mouse <- aggregate(z_score ~ MouseID + strainName + sex + DrugDose,
                         data = d2_df, FUN = mean, na.rm = TRUE)

# Pivot to wide format: one row per mouse, one z_score column per dose
z_wide <- reshape(z_per_mouse,
                  idvar   = c("MouseID", "strainName", "sex"),
                  timevar = "DrugDose",
                  direction = "wide")

# Rename z_score.20 -> z_score_20mgL etc.
names(z_wide) <- gsub("z_score\\.", "z_score_", names(z_wide))
names(z_wide) <- paste0(names(z_wide), ifelse(grepl("^z_score_", names(z_wide)), "mgL", ""))

# Keep only mice where any dose z-score exceeds threshold
dose_cols <- grep("^z_score_", names(z_wide), value = TRUE)
z_wide$max_abs_zscore <- apply(abs(z_wide[, dose_cols, drop = FALSE]), 1,
                                max, na.rm = TRUE)
outlier_mice <- z_wide[!is.na(z_wide$max_abs_zscore) &
                         z_wide$max_abs_zscore > ZSCORE_THRESHOLD, ]
outlier_mice <- outlier_mice[order(outlier_mice$max_abs_zscore, decreasing = TRUE), ]

write.csv(outlier_mice,
          file = file.path(outputDir, "Outliers_Zscore_gt2.5.csv"),
          row.names = FALSE)

cat("Outlier MouseIDs (|z| >", ZSCORE_THRESHOLD, "):\n")
print(outlier_mice$MouseID)
cat("\nTotal outlier mice:", nrow(outlier_mice), "\n")
cat("CSV written to:", file.path(outputDir, "Outliers_Zscore_gt2.5.csv"), "\n")
```