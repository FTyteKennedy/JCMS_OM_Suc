...
d1 <- data.table::as.data.table(read_excel(filepath, sheet = "TOM_MasterDataTableQuery", guess_max = 10000), showWarnings = FALSE) #made this correction to convert to data.table.
...
# Calculate z-scores for DrugMassConsumed within each strain, sex, and dose
d2 <- d2 %>%
  group_by(strainName, sex, DrugDose) %>%
  mutate(
    consumption_zscore = (DrugMassConsumed - mean(DrugMassConsumed, na.rm = TRUE)) / sd(DrugMassConsumed, na.rm = TRUE),
    is_outlier = abs(consumption_zscore) > 2  # Using a threshold of 2
  ) %>%
  ungroup()

# Handle cases where sd is 0 (e.g., only one data point), which results in NaN for z-score
d2$consumption_zscore[is.nan(d2$consumption_zscore)] <- 0
d2$is_outlier[is.na(d2$is_outlier)] <- FALSE

# Report the number of outliers found
outlier_summary <- d2 %>%
  filter(is_outlier == TRUE) %>%
  count(strainName, sex, DrugDose)

if (nrow(outlier_summary) > 0) {
  print("Potential outliers identified:")
  print(outlier_summary)
} else {
  print("No outliers identified with a z-score threshold of 2.")
}
