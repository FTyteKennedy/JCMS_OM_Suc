...
filename <- "2024-02-09 TOM_MasterDataTableQuery v3.txt"
checkErrordir(wd)
...
```{r}
# Specify the text to exclude
included_strains <- c("B6NJ-Adgrb2-/- Cyfip2 S968F<J>", "B6NJ-Ajap1-/- Cyfip2-S968F<J>", "B6NJ-AU040320-/- Cyfip2-S968F<J> HSubVi18p", "B6NJ-Bcl11b+/- Cyfip2-S968F<J> H Lethal", "B6NJ-Gigyf2tm1b-/- Cyfip2-S968F<J>", "B6NJ-Cyfip2-S968F<J> (GET204)")

# Filter the data, including rows with the included text
d2 <- d2[strainName %in% included_strains, ]

## Fix certain names of line: commented out since don't need this strain
...
outlier_plot <- ggplot(d2, aes(x = as.factor(DrugDose), y = DrugMassConsumed, color = is_outlier)) +
  geom_boxplot(outlier.shape = NA, aes(group = interaction(DrugDose, is_outlier)), fill = NA) +
  geom_jitter(width = 0.2, alpha = 0.8) +
  scale_color_manual(values = c("black", "red"), name = "Outlier (Z > 2)") +
  facet_grid(sex ~ strainName, labeller = labeller(strainName = label_wrap_gen(width = 25))) +
  labs(
    title = "Outlier Visualization (Z-score > 2)",
    subtitle = "Red points are flagged as potential outliers",
    y = "mgMeth/gBW/day",
    x = "Drug Dose"
  ) +
  theme_bw()

# Save the outlier plot
ggsave(file.path(outputDir, "OM_Outlier_Visualization_Z2.pdf"), outlier_plot, width = 12, height = 8)

# To filter out outliers for subsequent analysis, you could uncomment the following line:
# d2_filtered <- d2 %>% filter(!is_outlier)
# Then use d2_filtered for all downstream plotting and stats. For now, we will keep them.
