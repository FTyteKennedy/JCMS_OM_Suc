...
wd <- "Y:\\kumarlab-new\\Fionna\\KOMP\\R Scripts\\SucOM_Code_2026\\Data"
...
outputDir <- file.path(dirname(wd),"Suc_Processed")
...
```{r}
# commented out and added more sure pathing -Madison
# setwd(wd)
# setwd(file.path(outputDir))

outFileNamePDF <- file.path(outputDir,"AllStrainsSUC.pdf")
...
# Commented otu and added more sure pathing -Madison
# setwd(wd)
# setwd(file.path(outputDir))

outFileNamePDF <- file.path(outputDir,"SucPrefIndividualStrains.pdf") 
...
# QC: Per-Animal Sucrose Preference by Strain
Each page shows one strain with every mouse drawn as an individually colored line
across all sucrose doses (0 → 0.25% → 0.5% → 2%). Use the Mouse ID legend on the
right to identify any animals with outlier preference scores.

```{r}
strainList <- levels(as.factor(d2$strainName))

outFileNamePDF <- file.path(outputDir, "SucPref_MouseID_AllDoses.pdf")
...
# QC: Raw Consumption Volumes per Animal by Strain
Each page shows one strain. Each mouse gets its own panel with sucrose and water
volume consumed plotted as separate colored lines across sucrose doses.

```{r}
strainList <- levels(as.factor(d2$strainName))

outFileNamePDF <- file.path(outputDir, "SucRawVolumes_PerAnimal.pdf")
...
# Commented out and addded more sure pathing -Madison
# setwd(wd)
# setwd(file.path(outputDir))
...
# removed an errant comma -Madison 
p1 <- ggplot(toPlot1, aes(x = as.factor(DrugDose), y = DrugPreferenceScore , fill=as.factor(DrugDose))) +
      geom_boxplot(outlier.shape = NA, color="gray10", width = 0.65) +
...
outFileNamePDF <- file.path(outputDir,"AllStrainsLine_Sucrose.pdf") 
...
outFileNamePDF <- file.path(outputDir,"IndividualStrainLinePlot_Sucrose.pdf") 
...
outFileNamePDF <- file.path(outputDir,"IndividualStrainLinePlotPvalues_Suc.pdf") 
...
outFileNamePDF <- file.path(outputDir,"IndividualStrainLinePlotSexPvalues_Suc.pdf") 
